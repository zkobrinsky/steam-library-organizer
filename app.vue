      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          <div class="flex items-center space-x-4">
            <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Steam Library Organizer</h1>
            <span v-if="games.length > 0" class="text-sm text-gray-600 dark:text-gray-400">
          ...
          <div class="flex items-center space-x-4">
            ...
            <div v-if="steamUser" class="flex items-center space-x-3 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2">
              <img 
              ...
              <div class="hidden sm:block">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">
                  {{ formatSteamId(steamUser.steamId) }}
                </p>
              </div>
            </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="!isConfigured" class="text-center py-16">
        <div class="max-w-md mx-auto">
          ...
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">Welcome to Steam Library Organizer</h2>
          <p class="text-gray-600 dark:text-gray-400 mb-8">
            Connect your Steam account to analyze your library, get AI-powered game recommendations, and organize your collection with smart filtering tools.
          </p>
          <button 
      ...
      <!-- Main Content -->
      <div v-else>
        <div v-if="error" class="bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-600 text-red-700 dark:text-red-200 px-4 py-3 rounded mb-6">
          {{ error }}
        </div>
        ...
        <div v-if="loading" class="text-center py-12">
          ...
          <p class="mt-2 text-gray-600 dark:text-gray-400">Loading your Steam library...</p>
        </div>

        <div v-else-if="games.length === 0" class="text-center py-12">
          <p class="text-gray-600 dark:text-gray-400">
            No games found. Make sure your Steam profile is public and try refreshing.
          </p>
        </div>
...
const filteredGames = computed(() => {
  ...
  if (filters.value.playtime) {
    result = result.filter(game => {
      ...
      switch (filters.value.playtime) {
        case 'unplayed': return hours === 0
        case 'under1h': return hours > 0 && hours < 1
        case '1-10h': return hours >= 1 && hours < 10
        case '10-50h': return hours >= 10 && hours < 50
        case '50-100h': return hours >= 50 && hours < 100
        case 'over100h': return hours >= 100
        default: return true
      }
    })
  }
  ...
  if (filters.value.recentActivity) {
    result = result.filter(game => {
      ...
      return filters.value.recentActivity === 'recent' ? hasRecentActivity : !hasRecentActivity
    })
  }
  ...
  if (filters.value.collection) {
    if (filters.value.collection === 'uncategorized') {
      result = result.filter(game => 
        collectionsStore.getCollectionsByGame(game.appid).length === 0
      )
    } else if (filters.value.collection.startsWith('auto-')) {
      // Handle auto-collections
      const gamesInAutoCollection = collectionsStore.getGamesInCollection(filters.value.collection, games.value)
      const gameIds = new Set(gamesInAutoCollection.map(g => g.appid))
      result = result.filter(game => gameIds.has(game.appid))
    } else {
      // Handle custom collections
      result = result.filter(game => 
        collectionsStore.getCollectionsByGame(game.appid).some(c => c.id === filters.value.collection)
      )
    }
  }
